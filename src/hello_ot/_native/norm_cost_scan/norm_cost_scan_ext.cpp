...
std::vector<torch::Tensor> fused_gcost_topk(
        ...
        int64_t k,
        int64_t cost_type) {
    return fused_gcost_topk_cuda(query, db, dual_db, k, cost_type);
}

// CN: 证书归约入口：一次全矩阵遍历返回 (numerator, denominator)，
//     numerator = sum relu(u + v - cost)^2，
//     denominator = sum cost^2。语义与 Python 调度层一致。
...
//     numerator = sum relu(u + v - cost)^2 and denominator = sum cost^2,
//     matching the Python dispatch layer.
...
std::vector<torch::Tensor> fused_gcost_bidir_certificate(
        ...
        int64_t k,
        int64_t cost_type) {
    return fused_gcost_bidir_certificate_cuda(query, db, query_dual, db_dual, k, cost_type);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
            "fused_gcost_topk",
            &fused_gcost_topk,
            "Fused CUDA top-k for general metric costs (l1/linf/l2) with on-the-fly distance and dual subtraction");
    m.def(
            "fused_gcost_certificate",
            &fused_gcost_certificate,
            "Fused CUDA dual-feasibility certificate (numerator/denominator reductions) for general metric costs; set compute_denominator=0 to skip the denominator reduction");
    m.def(
